...
from utils.loss import MultipleKernelMaximumMeanDiscrepancy, GaussianKernel, FocalLoss
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        # 测试lmmd损失
        source = features_source  # 源域数据
        target = features_target  # 目标域数据
        s_label = labels_source  # 目标域数据源域真实标签
        t_label_new = outputs_target  # 目标域预测标签
        if args.mmd_type == 'lmmd':
            loss_mmd = lmmd.LMMD_loss(source, target, s_label, t_label_new, args)
        if args.mmd_type == 'lmmjp':
            loss_mmd = lmmd_jp.LMMD_loss(source, target, s_label, t_label_new, args)
        if args.mmd_type == 'lmmdjp':
            loss_mmd1, loss_mmd2 = lmmd_djp.LMMD_loss(source, target, s_label, t_label_new, args)
            loss_mmd = loss_mmd1 - 0.1 * loss_mmd2
        if args.mmd_type == 'djpmmd':
            loss_mmd = mk_djpmmd.raw_djpmmd(source, target, s_label, t_label_new, args)
        if args.mmd_type == 'mk-djpmmd':
            loss_mmd = mk_djpmmd.mk_djpmmd(source, target, s_label, t_label_new, args)
        if args.mmd_type == 'mmd':
            loss_mmd = mk_djpmmd.rbf_mmd(source, target, args)
        if args.mmd_type == 'mk-mmd':
            from mk_djpmmd import GaussianKernel
            from mk_djpmmd import MultipleKernelMaximumMeanDiscrepancy
            mkmmd_loss = MultipleKernelMaximumMeanDiscrepancy(
                kernels=[GaussianKernel(alpha=2 ** k) for k in range(-3, 2)],
                linear=not False
            )
            loss_mmd = mkmmd_loss(source, target)

        param = 1.0
        if args.mmd_type == 'lmmdjp':
            total_loss = classifier_loss + param * loss_mmd
            # loss = loss_cls - param * loss_mmd1
            # loss = loss_cls - param * loss_mmd2
        else:
            total_loss = classifier_loss + param * loss_mmd
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            print(log_str)
            print('loss_output: ', classifier_loss.item(), loss_mmd.item(), total_loss.item())  # output the loss
...
if __name__ == '__main__':
    ...
    for idt in range(n_subject):
        ...
        mmd_list = ['lmmd', 'lmmjp', 'lmmdjp', 'djpmmd', 'mk-djpmmd', 'mmd']
        args.mmd_type = mmd_list[1]
        args.method = args.mmd_type
