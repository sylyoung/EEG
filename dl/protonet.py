...
from utils.utils import lr_scheduler_full, fix_random_seed, cal_acc_comb, data_loader, cal_bca_comb, cal_metrics_ms, \
    cal_acc_msmm
    ...
    while iter_num < max_iter:
        ...
        for batch_id in range(len(dset_loaders['sources'][0])):
            ...
            for source_id in range(args.N - 1):
                ...
                sort = torch.sort(labels_source)
                features_source = features_source.squeeze(0)[sort.indices].squeeze(0)
                labels_source = labels_source.squeeze(0)[sort.indices].squeeze(0)
                ar_unique, cnts_class = torch.unique(labels_source, return_counts=True)

                # assumes two classes
                indices = np.zeros(features_source.size(0), dtype=bool)
                for set_id in range(cnts_class[0].item() // 2):
                    indices[set_id] = True
                for set_id in range(cnts_class[1].item() // 2):
                    indices[set_id + cnts_class[0]] = True
                if random.random() > 0.5:
                    indices = ~indices
                support_indices = torch.from_numpy(indices)
                query_indices = torch.from_numpy(~indices)
                support_features = features_source[support_indices]
                support_labels = labels_source[support_indices]
                query_features = features_source[query_indices]
                query_labels = labels_source[query_indices]

                center_0 = support_features[torch.where(support_labels == 0)[0]].mean(dim=0)
                center_1 = support_features[torch.where(support_labels == 1)[0]].mean(dim=0)
                class_centers = torch.stack((center_0, center_1))

                logits = pairwise_distances_logits(query_features, class_centers)
                loss = torch.nn.functional.cross_entropy(logits, query_labels)

                all_center_0 = features_source[torch.where(labels_source == 0)[0]].mean(dim=0)
                all_center_1 = features_source[torch.where(labels_source == 1)[0]].mean(dim=0)

                all_centers.append(all_center_0)
                all_centers.append(all_center_1)

                """
                source_class_centers = []

                for class_label in range(args.class_num):
                    indices = torch.where(labels_source == class_label)
                    features_class = features_source[indices[0]]
                    # assumes each batch contain samples from each class
                    split_ind = len(features_class) // 2

                    if random.random() > 0.5:
                        inputs_support = features_class[:split_ind]
                        inputs_query = features_class[split_ind:]
                    else:
                        inputs_support = features_class[split_ind:]
                        inputs_query = features_class[:split_ind]

                    source_class_center = inputs_support.mean(dim=0)
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            #print('all_centers.shape', all_centers.shape)
            #score = cal_metrics_ms(dset_loaders["Target"], netF, netCs, args.N, args.class_num, metrics)
            score = cal_acc_msmm(dset_loaders["Target"], netF, all_centers, metrics)
