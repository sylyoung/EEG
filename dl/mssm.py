...
# @Author  : Siyang Li
# @File    : mssm.py
# @Desc    : Multi-Source Single-Model
import numpy as np
import argparse
import os
import torch as tr
...
from utils.utils import lr_scheduler_full, fix_random_seed, cal_acc_comb, data_loader, cal_bca_comb, cal_metrics_ms, cal_acc_ms_distance, cal_bca_ms_distance
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        for batch_id in range(len(dset_loaders['sources'][0])):
            ...
            features_source_all, outputs_source_all = base_network(inputs_source_all)
            #features_target, outputs_target = base_network(inputs_target)

            #features_target = netC(inputs_target)
            #outputs_target = netC(features_target)

            '''
            outputs_source_list = torch.split(outputs_source_all, args.N - 1)
            labels_source_list = torch.split(labels_source_all, args.N - 1)
            loss_ce_per_domain = []
            for source_id in range(args.N - 1):
                classifier_loss_source = criterion(outputs_source_list[source_id], labels_source_list[source_id])
                loss_ce_per_domain.append(classifier_loss_source)
            loss_ce_per_domain = torch.stack(loss_ce_per_domain).reshape(-1)
            ...
            total_loss = loss_ce + args.trade_off * loss_penalty
            '''

            total_loss = criterion(outputs_source_all, labels_source_all)
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.paradigm == 'MI':
                ...
                log_str = 'Task: {}, Iter:{}/{}; BCA = {:.2f}%'.format(args.task_str,
                                                                       int(iter_num // len(dset_loaders["sources"][0])),
                                                                       int(max_iter // len(dset_loaders["sources"][0])),
                                                                       acc_t_te)
            input('')
