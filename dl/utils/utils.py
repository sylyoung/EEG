...
try:
    from alg_utils import EA, EA_online
...
def TTA(loader, model, args, balanced=True, flag=True, fc=None):
    ...
    for i in range(len(loader)):
        ...
        inputs = data[0]
        labels = data[1]
        inputs = inputs.reshape(1, 1, inputs.shape[-2], inputs.shape[-1])
        if i == 0:
            data_cum = inputs.float().cpu()
            labels_cum = labels.float().cpu()
        else:
    ...
    for i in range(len(loader)):
        ...
        if (i + 1) >= args.test_batch:
            ...
            for step in range(args.steps):
                ...
                loss = tr.mean(Entropy(nn.Softmax(dim=1)(model(inputs)[1].float().cpu() / args.t)))  # use this loss for any training statistics
                loss.backward()
                optimizer.first_step(zero_grad=True)

                # second forward-backward pass
                tr.mean(Entropy(nn.Softmax(dim=1)(model(inputs)[1].float().cpu() / args.t))).backward()  # make sure to do a full forward pass
...
def cal_bca_comb(loader, model, args, flag=True, fc=None):
    start_test = True
    with tr.no_grad():
        iter_test = iter(loader)
        for i in range(len(loader)):
...
def data_loader(Xs=None, Ys=None, Xt=None, Yt=None, args=None):
    ...
    dset_loaders["Target-Online-Imbalanced"] = Data.DataLoader(data_tar_imb, batch_size=1, shuffle=True, drop_last=False)  # shuffle to prevent sequences of same class
    ...
    if args.class_num == 2 and args.paradigm == 'MI': # only implemented for class-balanced for now
        # source
        indices = []
        for i in range(args.N - 1):
