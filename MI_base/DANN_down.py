...
def train_target(args):
    ...
    X_src, y_src = apply_randdown(X_src, y_src)
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.paradigm == 'MI':
                ...
                log_str = 'DOWN: Task: {}, Iter:{}/{}; Acc = {:.2f}%'.format(args.task_str, int(iter_num // len(dset_loaders["source"])), int(max_iter // len(dset_loaders["source"])), acc_t_te)
                args.log.record(log_str)
                print(log_str)
            elif args.paradigm == 'ERP':
                acc_t_te = cal_bca_comb(dset_loaders["Target"], base_network)
                log_str = 'DOWN: Task: {}, Iter:{}/{}; BCA = {:.2f}%'.format(args.task_str, int(iter_num // len(dset_loaders["source"])), int(max_iter // len(dset_loaders["source"])), acc_t_te)
                args.log.record(log_str)
                print(log_str)
...
if __name__ == '__main__':
    ...
    for data_name in data_name_list:
        ...
        args = argparse.Namespace(feature_deep_dim=feature_deep_dim, lr=0.0001, lr_decay1=0.1, lr_decay2=0.1,
                                  epsilon=1e-05, trial_num=trial_num, layer='wn',
