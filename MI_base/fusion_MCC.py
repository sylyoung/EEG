...
def train_target(args):
    ...
    #print('applying z-score X_src_fea:', X_src_fea.shape, ' X_tar_fea:', X_tar_fea.shape)
    #X_src_fea, X_tar_fea = apply_zscore(X_src_fea, X_tar_fea, args.N)
    ...
    a = []
    inds_feature = ranking[(-1 * args.feature_num):]
    for k in range(args.feature_num):
        a.append(np.arange(args.chn, dtype=int) + inds_feature[k] * args.chn)
    a = np.concatenate(a)

    X_src_fea = X_src_fea[:, a]
    X_tar_fea = X_tar_fea[:, a]

    X_src_fea = X_src_fea.reshape(X_src_fea.shape[0], args.chn, args.feature_num)
    X_tar_fea = X_tar_fea.reshape(X_tar_fea.shape[0], args.chn, args.feature_num)
    print('after feature selection X_src_fea:', X_src_fea.shape, ' X_tar_fea:', X_tar_fea.shape)
    '''
