...
from utils.utils import fix_random_seed, cal_acc_comb, data_loader, cal_bca_comb, cal_acc_online, TTA_ensemble, TTA_ensemble_distill
...
def train_target(args):
    ...
    netF, netC = network.backbone_net(args, return_type='xy')
    netF, netC = netF.cuda(), netC.cuda()
    student_network = nn.Sequential(netF, netC)

    for i in range(len(args.SEED)):
        base_networks[i].load_state_dict(torch.load('./runs/' + str(args.data_name) + '/' + str(args.backbone) +
                                                '_S' + str(args.idt) + '_seed' + str(args.SEED[i]) + '.ckpt'))


    if args.paradigm == 'MI':
        acc_t_te = TTA_ensemble_distill(dset_loaders["Target-Online"], base_networks, student_network, args)
        log_str = 'Task: {}, TTA Acc = {:.2f}%'.format(args.task_str, acc_t_te)
        args.log.record(log_str)
        print(log_str)
    elif args.paradigm == 'ERP':
        acc_t_te = TTA_ensemble_distill(dset_loaders["Target-Online"], base_networks, student_network, args, balanced=False)
...
if __name__ == '__main__':
    ...
    for data_name in data_name_list:
        ...
        args.data = data_name
        args.method = 'TTA-IM-Ens-Distill'
        ...
        subject_mean = sub_acc_all
        total_mean = np.round(np.mean(sub_acc_all), 3)
        total_std = 0
        ...
        result_dct = {'dataset': data_name, 'avg': total_mean, 'std': 0}
