...
class ConvChannelWise(nn.Module):
    def __init__(self, nn_deep, nn_out, in_channels=1, out_channels=1, bias=False, layer=1):
        super(ConvChannelWise, self).__init__()
        self.conv1 = nn.Conv1d(in_channels=in_channels, out_channels=out_channels, kernel_size=1, stride=1, padding=0, bias=bias)
        self.bn1 = nn.BatchNorm1d(num_features=out_channels * nn_deep)
        self.do1 = nn.Dropout(p=0.25)
        self.conv2 = nn.Conv1d(in_channels=1, out_channels=1, kernel_size=1, stride=1, padding=0,
                               bias=bias)
        self.bn2 = nn.BatchNorm1d(num_features=out_channels * nn_deep)
        self.do2 = nn.Dropout(p=0.25)

        self.layer = layer

        self.fc1 = nn.Linear(out_channels * nn_deep, 10)
