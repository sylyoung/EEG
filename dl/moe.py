...
def train_target(args):
    ...
    netF, _ = network.backbone_net(args, return_type='y')
    netF = netF.cuda()

    netC = MoE(input_size=args.feature_deep_dim, output_size=args.class_num,
               num_experts=args.N-1, noisy_gating=True, k=3, expert_model=None)
    netC.cuda()
    ...
    while iter_num < max_iter:
        ...
        predictions, aux_loss = base_network(inputs_source)
        _, predicted = torch.max(predictions.cpu().data, 1)
        loss = criterion(predictions, labels_source.reshape(-1, ))
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.paradigm == 'MI':
                acc_t_te = cal_acc_comb(dset_loaders["Target"], base_network, flag=False, fc=True)
                ...
                acc_t_te = cal_bca_comb(dset_loaders["Target"], base_network, flag=False, fc=True)
