...
from utils.utils import fix_random_seed, cal_acc_comb, data_loader, cal_bca_comb, cal_acc_online, TTA_LAME
...
def train_target(args):
    ...
    if args.paradigm == 'MI':
        acc_t_te = TTA_LAME(dset_loaders["Target-Online"], base_network, args)
        log_str = 'Task: {}, TTA Acc = {:.2f}%'.format(args.task_str, acc_t_te)
        args.log.record(log_str)
        print(log_str)
    elif args.paradigm == 'ERP':
        acc_t_te = TTA_LAME(dset_loaders["Target-Online"], base_network, args, balanced=False)
