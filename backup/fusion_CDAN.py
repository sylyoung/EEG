...
def train_target(args):
    ...
    ad_net_k = network.AdversarialNetwork(args.feature_deep_dim // 2, 32, 8).cuda()
    random_layer_k = RandomLayer([args.feature_deep_dim // 2, args.class_num], args.feature_deep_dim // 2)
    random_layer_k.cuda()

    ad_net_d = network.AdversarialNetwork(args.feature_deep_dim // 2, 32, 8).cuda()
    random_layer_d = RandomLayer([args.feature_deep_dim // 2, args.class_num], args.feature_deep_dim // 2)
    random_layer_d.cuda()
    ...
    optimizer_dk = optim.SGD(ad_net_k.parameters(), lr=args.lr)
    optimizer_dd = optim.SGD(ad_net_d.parameters(), lr=args.lr)
    ...
    while iter_num < max_iter:
        ...
        lr_scheduler_full(optimizer_dk, init_lr=args.lr, iter_num=iter_num, max_iter=args.max_iter)
        lr_scheduler_full(optimizer_dd, init_lr=args.lr, iter_num=iter_num, max_iter=args.max_iter)
        ...
        k_source, d_source = features_source
        k_target, d_target = features_target
        k_features = tr.cat((k_source, k_target), dim=0)
        d_features = tr.cat((d_source, d_target), dim=0)

        # new version img loss
        args.loss_trade_off = 0.5  # divided by half
        outputs = tr.cat((outputs_source, outputs_target), dim=0)
        softmax_out = nn.Softmax(dim=1)(outputs)
        entropy = Entropy(softmax_out)
        transfer_loss_k = CDANE([k_features, softmax_out], ad_net_k, entropy, calc_coeff(iter_num), random_layer=random_layer_k)
        transfer_loss_d = CDANE([k_features, softmax_out], ad_net_d, entropy, calc_coeff(iter_num), random_layer=random_layer_d)
        classifier_loss = criterion(outputs_source, labels_source)
        total_loss = args.loss_trade_off * transfer_loss_k + args.loss_trade_off * transfer_loss_d + classifier_loss

        optimizer_f.zero_grad()
        optimizer_c.zero_grad()
        optimizer_dk.zero_grad()
