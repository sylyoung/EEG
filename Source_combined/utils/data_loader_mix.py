...
def read_seizure_combine_tar(args):  # no data augment
    ...
    if args.data_env == 'gpu':
        domains = next(walk('/home/zwwang/code/Source_combined/data/mix_fts_labels/'), (None, None, []))[2]
        i = args.idt
        src_x = []
        src_y = []
        for j in range(len(domains)):
            if i != j:
                src = loadmat('/home/zwwang/code/Source_combined/data/mix_fts_labels/' + domains[j])
        ...
        # print(src_data.shape, src_label.shape)  # (n_src, chns, fts)
        # load target domain
        tar = loadmat('/home/zwwang/code/Source_combined/data/mix_fts_labels/' + domains[i])
