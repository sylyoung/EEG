...
from utils.dataloader import read_mi_test, read_seed_test, read_seizure_combine_srcbest
from utils.utils import lr_scheduler, fix_random_seed, op_copy, cal_acc, data_loader
...
def train_target(args):
    if args.data in ['SEED', 'SEED4']:
        ...
        X_src, y_src, X_tar, y_tar, X_sb, y_sb, trade_off = read_seizure_combine_srcbest(args)
    else:
    ...
    while iter_num < max_iter:
        ...
        try:
            ...
            source_loader_iter = iter(dset_loaders_sb["source"])
            inputs_source, labels_source = source_loader_iter.next()
        ...
        # src best loss
        sb_loss = lmmd(source=inputs_source, target=inputs_sb, s_label=labels_source, t_label=labels_sb)
        args.loss_trade_off = trade_off
        total_loss = classifier_loss + args.loss_trade_off * sb_loss
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
