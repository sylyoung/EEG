...
class CustomImagesDataset(Dataset):
    ...
    def __init__(self, csv_file, transform=None):
        ...
        if isinstance(csv_file, list):
            ...
            for csv_f in csv_file:
                csv = pd.read_csv(csv_f, header=None, sep=',')
                csvs.append(csv)
            self.csv_file = pd.concat(csvs)
        else:
    ...
    def getdataset(self):
        ...
        all_imgs, all_labels = torch.stack(all_imgs), torch.stack(all_labels)
...
def compute_mean_std_multisource(path, img_size):
    ...
    for p in path:
        ...
        dataset = CustomImagesDataset(p, transform=transform)
        img, _ = dataset.getdataset()
        imgs.append(img)
...
def apply_EA(path, img_size):
    ...
    imgs, labels = dataset.getdataset()
    imgs = imgs.numpy()
...
def image_classification(config):
    ...
    if dataset == 'Office31' or dataset == 'Office-Home' or dataset == 'ImageCLEF':
        if 'EA' in approach:
            print('applying EA...')
            imgs_train_all, labels_train_all = [], []
            for train_path in train_paths:
                imgs_train, labels_train = apply_EA(train_path, img_size=224)
                imgs_train_all.append(imgs_train)
                labels_train_all.append(labels_train)
            imgs_train_all, labels_train_all = np.concatenate(imgs_train_all), np.concatenate(labels_train_all)
            imgs_test, labels_test = apply_EA(test_paths, img_size=224)
            imgs_train_all, imgs_test = np.transpose(imgs_train_all, (0, 2, 3, 1)), np.transpose(imgs_test, (0, 2, 3, 1))
            train_data = EACustomImagesDataset(imgs_train_all, labels_train_all, transform=train_transform)
            test_data = EACustomImagesDataset(imgs_test, labels_test, transform=test_transform)
        else:
            train_data = CustomImagesDataset(train_paths, transform=train_transform)
            test_data = CustomImagesDataset(test_paths, transform=test_transform)
            ...
            if epoch + 1 == num_iter: # TODO
                ...
                if type(model) is list:
                    torch.save(model[0].state_dict(), './runs/' + dataset + '/model_feature_epoch_' + str(
                        epoch + 1) + '_' + exp_time + exp_id + '_UDA_' + str(source_domain) + 'to' + target_domain + '.pt')
...
if __name__ == '__main__':
    ...
    config['exp_id'] = '_25'  # experiment ID
    config['exp_time'] = str(date.today())  # experiment time
    config['dataset'] = 'Office-Home'  # dataset name string
    ...
    config['num_iter'] = 100  # number of max iterations
    config['batch_size'] = 64  # training batch size
    config['num_workers'] = 8  # number of parallel workers
    config['pretrained'] = False  # use ImageNet pretrained weights
    ...
    if config['dataset'] == 'Office31':
        source_domains = [['W', 'D'], ['A', 'D'], ['A', 'W']]
        target_domains = ['A', 'W', 'D']
    elif config['dataset'] == 'Office-Home':
        source_domains = [['Cl', 'Pr', 'Re'], ['Ar', 'Pr', 'Re'], ['Ar', 'Cl', 'Re'], ['Ar', 'Cl', 'Pr']]
        target_domains = ['Ar', 'Cl', 'Pr', 'Re']
    elif config['dataset'] == 'ImageCLEF':
        source_domains = [['c', 'i', 'p'], ['b', 'i', 'p'], ['b', 'c', 'p'], ['b', 'c', 'i']]
        target_domains = ['b', 'c', 'i', 'p']

    for source_domain in source_domains:
        for target_domain in target_domains:
            if target_domain in source_domain:
                continue
            ...
            for source_path in source_domain:
                source_csv = '/mnt/data2/sylyoung/Image/' + config['dataset'] + '/' + path_str + '/' + source_path + '.csv'
                train_path.append(source_csv)

            test_path = '/mnt/data2/sylyoung/Image/' + config['dataset'] + '/' + path_str + '/' + target_domain + '.csv'
            img_size = 224

            if 'EA' in config['approach']:
                mean, std = None, None
            else:
                #mean, std = compute_mean_std_multisource(train_path, img_size)  # training set mean and std for normalization
                mean = (0.6541134, 0.6337929, 0.61217713)  # Office-Home
                std = (0.3639425, 0.3654734, 0.3773743)  # Office-Home
            ...
            try:
                ...
                print('using GPU id' + str(cuda_device_id))
            except:
                try:
            ...
            task = str(source_domain) + 'to' + target_domain
