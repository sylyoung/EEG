...
def cal_weight(s_label, t_label, class_num=31):
    ...
    if len(com_class) == 0:
        ...
        # s_sum = len([i for i in range(len(s_sca_label)) if s_sca_label[i] in com_class])
        # t_sum = len([i for i in range(len(t_sca_label)) if t_sca_label[i] in com_class])
        ...
        if (len(src_class_own) == 0) | len(tar_class_own) == 0:
            ...
            s_vec_weight_dis = cal_basic_weight(s_vec_label_own_final, tmp_col_num)
            t_vec_weight_dis = cal_basic_weight(t_vec_label_own_final, tmp_col_num)
