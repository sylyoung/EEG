...
def train_target(args):
    ...
    optimizers = []
    for i in range(args.num_nets):
        optimizer = optim.Adam(base_networks[0].parameters(), lr=args.lr)
    ...
    while iter_num < max_iter:
        ...
        outputs_source_sum = torch.tensor([0., 0.]).cuda()
        classifier_losses = []

        for i in range(args.num_nets):
            features_source, outputs_source = base_networks[i](inputs_source)
            outputs_source_sum += outputs_source
            classifier_loss = criterion(outputs_source, labels_source)
            classifier_losses.append(classifier_loss)

        outputs_source_sum /= args.num_nets

        for i in range(args.num_nets):

            optimizers[i].zero_grad()
            discrepancy_loss = None
            loss = classifier_losses[i] + discrepancy_loss # TODO ?
            # discrepancy_loss promotes diversity of different outputs across ensembled models
            classifier_losses[i].backward()
            optimizers[i].step()
