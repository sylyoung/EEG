...
def read_ch_combine_srcsel(args):  # no data augment
    ...
    if args.data_env == 'gpu':
        domains = next(walk('/mnt4/zwwang/Source_combined/data/child_hos/fts_labels/'), (None, None, []))[2]
        src_sel_pd = pd.read_excel('/mnt4/zwwang/Source_combined/data/CHSZ_DSSS_v1.xlsx',
                                   index_col=0)
        src_sel_numpy = src_sel_pd.to_numpy()
        tar_num = src_sel_numpy[:, 0].shape[0]
        idt = args.idt
        tar = loadmat('/mnt4/zwwang/Source_combined/data/child_hos/fts_labels/' + domains[idt])
        tar_data, tar_label = tar['data'], tar['label']
        tar_label = tar_label.squeeze()
        for i in range(tar_num):
            if str(src_sel_numpy[:, 0][i]) == domains[idt][1:-4]:
                d = src_sel_numpy[i][1:args.sn]  # 选择源域数量一半
                src_num = d.shape[0]
                src_x = []
                src_y = []
                for j in range(src_num):
                    print('this is the ' + str(j + 1) + ' src: ', str(d[j]))
                    src = loadmat('/mnt4/zwwang/Source_combined/data/child_hos/fts_labels/' + 'S' + str(d[j]) + '.mat')
                    src0, src1 = src['data'], src['label']
                    src1 = src1.squeeze()
...
def read_ch_combine_srcbest_M(args):  # no data augment
...
def read_ch_combine_sbmada_M(args):  # no data augment
...
def read_seizure_combine_srcsel(args):  # no data augment
    ...
    if args.data_env == 'gpu':
        domains = next(walk('/mnt4/zwwang/Source_combined/data/fts_labels/'), (None, None, []))[2]
        src_sel_pd = pd.read_excel('/mnt4/zwwang/Source_combined/data/NICU_DSSS_v1.xlsx',
                                   index_col=0)
        src_sel_numpy = src_sel_pd.to_numpy()
        tar_num = src_sel_numpy[:, 0].shape[0]
        idt = args.idt
        tar = loadmat('/mnt4/zwwang/Source_combined/data/fts_labels/' + domains[idt])
        tar_data, tar_label = tar['data'], tar['label']
        tar_label = tar_label.squeeze()
        for i in range(tar_num):
            if str(src_sel_numpy[:, 0][i]) == domains[idt][1:-4]:
                ...
                for j in range(src_num):
                    print('this is the ' + str(j + 1) + ' src: ', str(d[j]))
                    src = loadmat('/mnt4/zwwang/Source_combined/data/fts_labels/' + 'S' + str(d[j]) + '.mat')
                    src0, src1 = src['data'], src['label']
                    src1 = src1.squeeze()
                    # 先smote再combine
                    oversample = SMOTE(random_state=42)
...
def read_ch_combine_srcbest(args):  # no data augment
    ...
    if args.data_env == 'gpu':
        domains = next(walk('/mnt2/zwwang/Source_combined/data/child_hos/fts_labels/'), (None, None, []))[2]
        src_sel_pd = pd.read_excel('/mnt2/zwwang/Source_combined/data/CHSZ_DSSS_v1.xlsx',
                                   index_col=0)
        src_sel_numpy = src_sel_pd.to_numpy()
        tar_num = src_sel_numpy[:, 0].shape[0]
        # input trade off
        corre_pd = pd.read_excel('/mnt2/zwwang/Source_combined/data/CHSZ_tradeoff_v1.xlsx', index_col=0)
        corre_numpy = corre_pd.to_numpy()
        idt = args.idt
        tar = loadmat('/mnt2/zwwang/Source_combined/data/child_hos/fts_labels/' + domains[idt])
        tar_data, tar_label = tar['data'], tar['label']
        tar_label = tar_label.squeeze()
        for i in range(tar_num):
            if str(int(corre_numpy[i][0])) == domains[idt][1:-4]:
            ...
            if str(src_sel_numpy[:, 0][i]) == domains[idt][1:-4]:
                d = src_sel_numpy[i][1:]  # 选择源域数量[all]
                sb = loadmat('/mnt2/zwwang/Source_combined/data/child_hos/fts_labels/' + 'S' + str(d[0]) + '.mat')
                sb_data, sb_label = sb['data'], sb['label']
                sb_label = sb_label.squeeze()
                src_num = d.shape[0]
                src_x = []
                ...
                for j in range(src_num):
                    print('this is the ' + str(j + 1) + ' src: ', str(d[j]))
                    src = loadmat('/mnt2/zwwang/Source_combined/data/child_hos/fts_labels/' + 'S' + str(d[j]) + '.mat')
                    src0, src1 = src['data'], src['label']
                    src1 = src1.squeeze()
                    # 下采样
                    # under = RandomUnderSampler(sampling_strategy='majority')
...
def read_ch_combine_sbmada(args):  # no data augment
    ...
    if args.data_env == 'gpu':
        domains = next(walk('/mnt2/zwwang/Source_combined/data/child_hos/fts_labels/'), (None, None, []))[2]
        src_sel_pd = pd.read_excel('/mnt2/zwwang/Source_combined/data/ch_mada_no_smote.xlsx',
                                   index_col=0)
        src_sel_numpy = src_sel_pd.to_numpy()
        tar_num = src_sel_numpy[:, 0].shape[0]
        # input trade off
        corre_pd = pd.read_excel('/mnt2/zwwang/Source_combined/data/ch_trade_off.xlsx', index_col=0)
        corre_numpy = corre_pd.to_numpy()
        idt = args.idt
        tar = loadmat('/mnt2/zwwang/Source_combined/data/child_hos/fts_labels/' + domains[idt])
        tar_data, tar_label = tar['data'], tar['label']
...
def read_seizure_combine_srcbest(args):  # no data augment
    ...
    if args.data_env == 'gpu':
        domains = next(walk('/mnt2/zwwang/Source_combined/data/fts_labels/'), (None, None, []))[2]
        src_sel_pd = pd.read_excel('/mnt2/zwwang/Source_combined/data/mada_max10_all_without_smote.xlsx',
                                   index_col=0)
        src_sel_numpy = src_sel_pd.to_numpy()
        tar_num = src_sel_numpy[:, 0].shape[0]
        # input trade off
        corre_pd = pd.read_excel('/mnt2/zwwang/Source_combined/data/exact_sb_tradeoff.xlsx', index_col=0)
        corre_numpy = corre_pd.to_numpy()
        idt = args.idt
        tar = loadmat('/mnt2/zwwang/Source_combined/data/fts_labels/' + domains[idt])
        tar_data, tar_label = tar['data'], tar['label']
        tar_label = tar_label.squeeze()
        for i in range(tar_num):
            if str(int(corre_numpy[i][0])) == domains[idt][1:-4]:
            ...
            if str(src_sel_numpy[:, 0][i]) == domains[idt][1:-4]:
                d = src_sel_numpy[i][1:]  # 选择源域数量[all]
                sb = loadmat('/mnt2/zwwang/Source_combined/data/fts_labels/' + 'S' + str(d[0]) + '.mat')
                sb_data, sb_label = sb['data'], sb['label']
                sb_label = sb_label.squeeze()
                src_num = d.shape[0]
                src_x = []
                ...
                for j in range(src_num):
                    print('this is the ' + str(j + 1) + ' src: ', str(d[j]))
                    src = loadmat('/mnt2/zwwang/Source_combined/data/fts_labels/' + 'S' + str(d[j]) + '.mat')
                    src0, src1 = src['data'], src['label']
                    src1 = src1.squeeze()
                    # 先smote再combine
                    oversample = SMOTE(random_state=42)
