...
from utils.utils import fix_random_seed, cal_acc_comb, data_loader, cal_bca_comb, cal_acc_online
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num == max_iter:
            ...
            if args.paradigm == 'MI':
                acc_t_te = cal_acc_online(dset_loaders["Target-Online"], base_network, args)
                log_str = 'Task: {}, Iter:{}/{}; Acc = {:.2f}%'.format(args.task_str, int(iter_num // len(dset_loaders["source"])), int(max_iter // len(dset_loaders["source"])), acc_t_te)
                args.log.record(log_str)
                print(log_str)
            elif args.paradigm == 'ERP':
                acc_t_te = cal_acc_online(dset_loaders["Target-Online"], base_network, args, balanced=False)
                log_str = 'Task: {}, Iter:{}/{}; BCA = {:.2f}%'.format(args.task_str, int(iter_num // len(dset_loaders["source"])), int(max_iter // len(dset_loaders["source"])), acc_t_te)
                args.log.record(log_str)
                print(log_str)
...
if __name__ == '__main__':
    ...
    dct.to_csv('./logs/' + str(args.method) + ".csv", mode='a')