...
def image_classification(config):
    ...
    def train(model, iterator, optimizer, criterion, savelog=False, epoch=None, exp_time=None):
        ...
        for batch in iterator:
            ...
            if approach == 'MoE':
                #predictions, aux_loss = model[1](model[0](data))
                predictions, aux_loss = model(data)
    ...
    model_arc, mean, std, pretrained, train_all, validation, randaug, labelsmooth, approach \
    = \
        ...
        config['train_all'], config['validation'], config['randaug'], config['labelsmooth'], config['approach']
        ...
        if approach == 'MoE':
            ...
            if not train_all:
                for para in feature.parameters():
                    para.requires_grad = False
            feature.to(device)
            clf = MoE(input_size=num_ftrs, output_size=num_classes, num_experts=10, noisy_gating=True, k=4, expert_model=model)
            for para in clf.parameters():
                para.requires_grad = True
            clf.to(device)
            model = [feature, clf]
            ...
            for para in model.fc.parameters():
                para.requires_grad = True
            model = MoE(input_size=3 * 224 * 224, output_size=num_classes, num_experts=8, noisy_gating=True, k=3, expert_model=model)
            model.to(device)
            #for i in range(8):
            #    model.experts[i].to('cuda:' + str(2))
        else:
            feature = resnet50(pretrained=pretrained)
            num_ftrs = feature.fc.in_features
            feature.fc = nn.Flatten(start_dim=1, end_dim=-1)
    ...
    #elif dataset == 'Office31' or dataset == 'Office-Home' or dataset == 'VisDA':
...
if __name__ == '__main__':
    ...
    config['exp_id'] = '_14'  # experiment ID
    ...
    config['num_iter'] = 50  # number of max iterations
